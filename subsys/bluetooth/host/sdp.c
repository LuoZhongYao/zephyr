...
NET_BUF_POOL_DEFINE(sdp_max_att_pool, CONFIG_BT_MAX_CONN,
			...
			if ((!sad->state->pkt_full) &&
				...
				if (sad->state->rsp_buf_empty) {
					...
					if (att_buf != NULL) {
						...
						if (space >=
						    (att_buf->len - sad->state->last_att_offset)) {
							net_buf_add_mem(
								sad->rsp_buf,
								att_buf->data +
									sad->state->last_att_offset,
								(att_buf->len -
								 sad->state->last_att_offset));
							sad->att_list_len =
								att_buf->len -
...
int bt_sdp_register_service(struct bt_sdp_record *service)
{
	...
	*((uint32_t *)(service->attrs[0].val.data)) = handle;
	...
	BT_DBG("Service registered at %x", handle);

	return 0;
}

static void bt_sdp_repair(void)
{
	uint8_t index = 0;
	for (struct bt_sdp_record *rec = db; rec; rec = rec->next) {
		rec->index = index++;
	}
}
...
int bt_sdp_unregister_service(struct bt_sdp_record *service)
{
	...
	key = irq_lock();
	for (struct bt_sdp_record **cur = &db; *cur; cur = &(*cur)->next) {
		if (*cur != service) {
			continue;
		}
	...
	irq_unlock(key);
	BT_DBG("Service unregistered at %x", service->handle);
	return  -ENOENT;
}
