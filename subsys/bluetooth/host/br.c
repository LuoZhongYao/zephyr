...
void bt_hci_mode_change(struct net_buf *buf)
{
	...
	BT_DBG("mode %d, interval %d, status %02x", evt->mode,
	       sys_le16_to_cpu(evt->interval), evt->status);
	...
	conn->br.interval = sys_le16_to_cpu(evt->interval);
	notify_mode_changed(conn);
